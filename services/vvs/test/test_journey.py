...
class TestJourney(unittest.TestCase):
    ...
    def test_to_str_to_event_and_to_table(self):
        ...
        leg2 = Journey(
            "C", "B", start + timedelta(minutes=5), start + timedelta(minutes=15)
        )
            ...
            for line in ical.splitlines():
                ...
                if all(
                    [
                        ...
                        for ignore_field in non_indicative_ical_fields
                    ]
                ):
                    if len(line) > 0:
                        rest.append(line)
        ...
        with self.subTest("can transform into cal event"):
            ...
            for l_exp, l_act in zip(filter_ical(exp_ev), filter_ical(actual_ev)):
