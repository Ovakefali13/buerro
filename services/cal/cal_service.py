...
class iCloudCaldavRemote(CalRemote):
    def __init__(self, calendar_name: str = None):
        ...
        if not all([var in os.environ for var in required_env]):
            raise EnvironmentError(
                "Did not set all of these preferences: ", required_env
            )
        ...
        self.calendar = _get_named_calendar(principal.calendars(), calendar_name)
        if self.calendar is None:
            raise EnvironmentError(
                "Provided calendar could not be found", calendar_name
            )
    ...
    def from_caldav(self, caldav_events):
        def _ev_from_caldav(icloud_ev):
            if icloud_ev.vobject_instance:
                ...
                if obj.name.lower() == "vcalendar":
                    vevents = list(
                        filter(
                            lambda child: child.name.lower() == "vevent",
                            obj.getChildren(),
                        )
                    )
                    if len(vevents) == 0:
                    ...
                    if len(vevents) > 1:
                        raise Exception(
                            "Unexpectedly found two VEVENT in a " + "VCALENDER"
                        )
                    vevent = vevents[0]
        ...
        return list(
            filter(lambda e: e is not None, map(_ev_from_caldav, caldav_events))
        )
