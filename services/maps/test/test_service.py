...
class TestMapService(unittest.TestCase):
    ...
    def test_get_summary(self):
        summary = self.map_service.get_route_summary(
            self.dhbw, self.mensa, "driving-car"
        )
        self.assertIn('distance', summary)
        self.assertIn('duration', summary)
        self.assertEqual(summary['start'], self.dhbw)
        self.assertEqual(summary['dest'], self.mensa)

    def test_get_route_link(self):
        link = self.map_service.get_route_link(self.dhbw, self.mensa)
        self.assertEqual(
            link,
            "https://routing.openstreetmap.de/?loc=48.773563%2C9.170963&loc=48.780834%2C9.169989&hl=en&srv=1",
        )
...
class TestGeocodingService(unittest.TestCase):
    if "DONOTMOCK" in os.environ:
        geocoding_service = GeocodingService.instance(GeocodingJSONRemote.instance())
    else:
        geocoding_service = GeocodingService.instance(GeocodingMockRemote.instance())
