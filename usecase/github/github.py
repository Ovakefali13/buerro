...
class Github(Usecase):
        ...
        if self.issue:
            if self.state == 0:
                ...
                if "cal" in message:
                    ...
                    if time_slot:
                        ...
                        return Reply(
                            {
                                "message": "I have found time in your calendar at "
                                + str(time_slot["start"])
                                + ". Would you like to work on the issue at that time?"
                            }
                        )
                    else:
                        return Reply(
                            {
                                "message": "Sadly I could not find a free time slot in your calendar :/"
                            }
                        )
            ...
            if self.state == 1:
                if "yes" in message:
                    ...
                    return Reply(
                        {
                            "message": "Okay I will set an event in your calendar at that time :)"
                        }
                    )
                if "no" in message:
                    self.state = 0
                    ...
                    return Reply(
                        {
                            "message": "Okay maybe you want to look for a time yourself :)"
                        }
                    )
        else:
            pass
    ...
    def dispatch_proactive_notification_message(self, notification):
        ...
        try:
            ...
            print(
                "WARNING: Could not push notification. Maybe you are in test environment?"
            )
