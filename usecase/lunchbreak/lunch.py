...
from handler import Notification, NotificationHandler, LocationHandler, UsecaseStore
...
class Lunchbreak(Usecase):
    ...
    def advance(self, message):
        ...
        if not self.restaurants:
            restaurants, self.start, self.end, duration = self.check_lunch_options(
                coords
            )
            ...
            return_message = (
                f"Your lunch starts at {formated_lunch_time}. "
        ...
        if (
            not hasattr(self, "restaurants")
            or not self.restaurants
            or message == "proactive"
        ):
            ...
            if (choice is None) or (choice > self.max_restaurants):
                ...
                return Reply(
                    (
                        "I could not match your answer to any restaurant. "
                        "Please try again."
                    )
                )

            link = self.open_maps_route(coords, self.restaurants[choice])
            self.create_cal_event(
                self.lunch_start, self.lunch_end, self.restaurants[choice], link
            )
            ...
            return Reply(
                {
                    "message": f"I added your lunch at {restaurant_name} to your calendar. \nWith this link you can navigate there: ",
                    "link": link,
                }
            )
    ...
    def check_lunch_options(self, location: list):
        ...
        start = datetime.now(pytz.utc).replace(
        ...
        (
            ...
            self.lunch_start,
            self.lunch_end,
        ) = self.find_longest_timeslot_between_hours(start, end)
        ...
        hours_until_lunch = self.time_diff_in_hours(
            self.lunch_start, datetime.now(pytz.utc)
        )
        ...
        will_be_bad_weather = self.weather_adapter.will_be_bad_weather(
            hours_until_lunch
        )
        ...
        self.restaurants = self.yelp_service.get_short_information_of_restaurants(
            search_params
        )
    ...
    def evaluate_user_request(self, message, restaurants):
        ...
        try:
            ...
            if selectedRestaurant > 5:
                selectedRestaurant = 5
        except:
            pass  # no number
    ...
    def hours_until_lunch(self):
        ...
        duration, lunch_start, lunch_end = self.find_longest_timeslot_between_hours(
            start, end
        )
    ...
    def create_proactive_notification(self, message):
        ...
        notification.set_body(
            "Check out the best restaurant nearby for lunch. Just open the Buerro PDA!"
        )
